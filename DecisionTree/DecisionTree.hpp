	...
	*/
		...
		std::function<double(const std::vector<T2>&)> chaos = GiniIndex<T2>)	const
	{
		...
		*/
		...
		/*
		* claculate the highest frequency target in current data
		* return map{option, count}
		*/

		auto maxTarget = maxInMap(frequencyVector<T2>(target));
		root->target = maxTarget.first;

		/*
		* all target of features a common
		*/
		if (maxTarget.second == target.size())	return root;
		/*
		* there no more than ones feature for classify
		*/
		if (feature[0].size() == 1)	return root;
		...
		for (size_t i = 0; i < category; i++)
		{
			...
			*/
			...
			for (auto iter : uniqueValue)
			{
				...
				currentChaos += chaos(filter) * (1.0 * filter.size() / target.size());
			}
