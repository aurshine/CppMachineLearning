...
public:
	...
	*/
	std::vector<std::pair<double, T2>> KNearest(const T1& m_feature,
		int K = 5,
		std::function<double(const std::vector<featureType>&, const std::vector<featureType>&)> clacDistance = EuclideanDist<featureType>) const
	{
	...
	*/
		...
		std::function<double(const std::vector<featureType>&, const std::vector<featureType>&)> clacDistance = EuclideanDist<featureType>)	const
	{
		...
		for (size_t i = 0; i < features.size(); ++i)
			predictTargets[i] = this->predict(features[i], K, clacDistance);
